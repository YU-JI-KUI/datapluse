...
环境策略
────────
  dev  — console 彩色（ConsoleRenderer）+ access 日志也输出到 console，文件 JSON
  stg  — console JSON（受 LOG_LEVEL 控制），access 日志只写文件，文件 JSON
  prod — console JSON（受 LOG_LEVEL 控制），access 日志只写文件，文件 JSON
...
def setup_logging(settings: Any) -> None:
    ...
    console_handler.setLevel(level)  # 所有环境统一受 LOG_LEVEL 控制
