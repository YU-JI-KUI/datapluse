-- =============================================================================
-- Datapluse 数据库初始化脚本
-- 执行前请确认数据库已存在：CREATE DATABASE datapluse;
-- 执行方式：psql -h <host> -U <user> -d datapluse -f database/init.sql
-- =============================================================================

-- 扩展：uuid 生成（可选，本项目由应用层生成 UUID）
...
COMMENT ON COLUMN datasets.id         IS '数据集 UUID，由应用层生成';
...
CREATE TABLE IF NOT EXISTS data_items (
    id              VARCHAR(36)  NOT NULL,
    dataset_id      VARCHAR(36)  NOT NULL,       -- 所属数据集（强制关联）
...
COMMENT ON COLUMN data_items.conflict_detail IS
    ...
    {"similarity":0.93, "threshold":0.9, "paired_id":"uuid", "paired_text":"...",
...
-- 初始数据：默认数据集
-- =============================================================================
INSERT INTO datasets (id, name, description, is_active, created_at, updated_at)
VALUES
...
-- =============================================================================
-- 管理员账号说明
-- 请使用 tools/hash_password.py 生成 bcrypt 哈希后替换 <BCRYPT_HASH>
-- 示例：python tools/hash_password.py
-- =============================================================================
...
--
-- 或者直接运行：python tools/seed_admin.py
