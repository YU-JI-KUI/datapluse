...
- 全量运行（run_all）
- 单步运行（run_step）
- 状态追踪（写入 PostgreSQL pipeline_status 表）
- 详细进度：数量 / 百分比 / 速度 / ETA
"""
...
async def step_process() -> dict[str, Any]:
    """清洗 raw → processed"""
    db = get_db()
    raw_items = db.list_by_status("raw")
    total = len(raw_items)
    if total == 0:
...
async def step_pre_annotate() -> dict[str, Any]:
    ...
    items = db.list_by_status("processed")
    total = len(items)
    if total == 0:
...
async def step_embed() -> dict[str, Any]:
    """为 pre_annotated / labeled 数据生成 embedding"""
    db = get_db()
    emb = get_emb()
    items = []
    for s in ["pre_annotated", "labeled", "checked"]:
        items.extend(db.list_by_status(s))
...
async def step_check() -> dict[str, Any]:
    ...
    _set_status("running", "check", 100, detail={
        "total": total, "pct": "100%",
...
async def run_all() -> None:
    ...
    try:
        ...
        r3 = await step_embed()
        r4 = await step_check()
        _set_status("completed", "", 100, finished_at=_now(), results=[r1, r2, r3, r4])
...
async def run_step(step: str) -> dict[str, Any]:
    ...
    try:
        fn = {"process": step_process, "pre_annotate": step_pre_annotate,
              "embed": step_embed, "check": step_check}[step]
        result = await fn()
