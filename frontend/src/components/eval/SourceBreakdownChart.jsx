/** 评测来源分布：整份日志按「活动标问 / 短路规则 / AI 评测」拆分的横向柱状图。
...
const KIND_LABEL = { activity: '活动标问', rule: '短路规则', llm: 'AI 评测' }
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <SectionTitle hint="整份日志按处理来源拆分：活动标问(跳过)、短路规则(免LLM)、AI 评测">
          评测来源分布
        </SectionTitle>
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground mt-1">
          ...
          <span><i className="inline-block w-2.5 h-2.5 rounded-sm mr-1 align-middle" style={{ background: COLORS.rule }} />短路规则 {ruleTotal} 条（{fmtPct(grand ? ruleTotal / grand : 0)}，免 LLM）</span>
